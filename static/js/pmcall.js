...
async function pmvideostart() {
  if (document.querySelector(".pm_video_div").style.display == "block") {
    ...
    document.getElementById("notifyType").innerText =
      "you are already in call with this person!";
    ...
    document.querySelector(".video_wrapper2 label").innerText =
      document.querySelector(".pm_type").innerText;
    ...
    peer.on("signal", (data) => {
      if (data === "screenshare") {
        ...
        socket.emit("pmcall", {
          id: this_userid,
          name: username,
          rcallid: document.querySelector(".pmchathead").id,
          call: "video",
          signal: data,
        });
      }
...
async function acceptcall(event) {
  ...
  document.querySelector(".video_wrapper1 label").innerText = username;
  document.querySelector(".video_wrapper2 label").innerText =
    document.querySelector(".scall_name").innerText;

  pmchat(
    document.querySelector(".incoming_pmcall").id,
    document.querySelector(".scall_name").innerText,
    event
  );
...
async function declinecall() {
  socket.emit("callDeclined", {
    id: document.querySelector(".incoming_pmcall").id,
  });
  document
    .querySelector(".incoming_pmcall")
    .setAttribute("style", "display:none");
...
function enlarge() {
  ...
  document
    ...
    .setAttribute("style", "width: 50%;height: 70%;");
  document
